    ...
    CartesianTask::CartesianTask(rclcpp::Node::SharedPtr& node)
{
  ...
  RCLCPP_INFO(LOGGER, "Initializing task pipeline");
    ...
    void CartesianTask::setPlannerProperties(std::shared_ptr<moveit::task_constructor::solvers::PipelinePlanner> pipeline_planner, std::string planner_id, double velocity)
    {
        RCLCPP_INFO(LOGGER, "[CartesianTask instance]: setPlannerProperties");
        ...
        if (planner_plugin_ == "pilz_industrial_motion_planner::CommandPlanner")
        {
            ...
            else
            {
                ...
                if (velocity >= max_trans_vel_)
                {
                    ...
                    RCLCPP_ERROR_STREAM(LOGGER, "The desired Cartesian translational velocity is above Pilz Maximum, setting translation scaling factor to 1");
                }
                else
                ...
                if (cartesian_rot_velocity_ >= max_rot_vel_)
                {
                    ...
                    RCLCPP_INFO_STREAM(LOGGER, "The desired Cartesian rotational velocity is above Pilz Maximum, setting rotation scaling factor to 1");
                }
                else
                ...
                if (trans_velocity_scaling_factor > rot_velocity_scaling_factor)
                {
                    ...
                    RCLCPP_INFO_STREAM(LOGGER, "Setting scaling factor to fullfill desired/max Cartesian rotational velocity: " << rot_velocity_scaling_factor);
                }
                else
                {
                    pipeline_planner->setProperty("max_velocity_scaling_factor", trans_velocity_scaling_factor);
                    RCLCPP_INFO_STREAM(LOGGER, "Setting scaling factor to fullfill desired/max Cartesian translational velocity: " << trans_velocity_scaling_factor);
                }
            }
        }
        else if (planner_plugin_ == "ompl_interface/OMPLPlanner")
        {
            ...
            else
            {
                ...
                RCLCPP_INFO_STREAM(LOGGER, "Setting max " << welding_tcp_frame_ << " speed to fullfill desired Cartesian translational velocity [m/s]: " << velocity);
            }
     ...
     */
    void CartesianTask::addStage(std::string stage_caption, std::string manufacturingSubFrameID, std::string planner_id, double velocity)
    {
        RCLCPP_DEBUG(LOGGER, "[CartesianTask instance]: Add stage task_transform");
        ...
        // stage->setCartesianTask(manufacturingSubFrameID, revertDirection);
    ...
    /**
     * @brief Adds a stage to the task. Additionally allows to specify an explicit stage-specific offset. Does not take into account the task_transform_.
     * 
     * @param stage_caption How the stage is named
     * @param manufacturingSubFrameID The header.frame_id of the frame to reach in this stage
     ...
     * @param velocity The velocity which the robot should have
     */
    void CartesianTask::addStage(std::string stage_caption, std::string manufacturingSubFrameID, std::string manufacturingToTaskFrameID, geometry_msgs::msg::Transform stage_offset, std::string planner_id, double velocity)
    {
        RCLCPP_DEBUG(LOGGER, "[CartesianTask instance]: Add stage stage_offset");
        // Create an instance of the planner and set its properties
        auto pipeline_planner = std::make_shared<solvers::PipelinePlanner>();
        setPlannerProperties(pipeline_planner, planner_id, velocity);
